...
public class SignalComponentsEnhancer{
	...
	public SignalComponentsEnhancer(int movingAverage) {
		this.movingAverage = movingAverage;
	}

	SignalComponentData init(BhavData bhavData, BhavData bhavData_1){
		SignalComponentData signalComponents = new SignalComponentData();
	...
	SignalComponentData init(Integer magicNumber, List<BhavData> bhavDatas, List<SignalComponentData> signals){
		BhavData bhavData = bhavDatas.get(bhavDatas.size()-1);
		BhavData bhavData_1 = bhavDatas.get(bhavDatas.size()-2);
		SignalComponentData prevSignalComponents = signals.get(signals.size()-1);
		SignalComponentData signalComponents = new SignalComponentData();
		signalComponents.setSymbol(bhavData.getSymbol());
		signalComponents.setSignalDate(bhavData.getTimeStamp());
		Double TRnTmp = 0D;
		...
		for(SignalComponentData signalComponentsTmp: signals){
...
//		Double close = bhavData.getClose() * signalComponents.getSplitMultiplier();
		...
		Double delta = new Double(IntelliInvestStore.SIGNAL_DELTA);
...
//		System.out.println(signalComponents);
	...
	SignalComponentData init(Integer magicNumber, List<BhavData> bhavDatas, SignalComponentData prevSignalComponents){
		BhavData bhavData = bhavDatas.get(bhavDatas.size()-1);
		BhavData bhavData_1 = bhavDatas.get(bhavDatas.size()-2);
		SignalComponentData signalComponents = new SignalComponentData();
		signalComponents.setSymbol(bhavData.getSymbol());
...
//		Double close = bhavData.getClose() * signalComponents.getSplitMultiplier();
		...
		//TRn
		signalComponents.setTRn((prevSignalComponents.getTRn() * (movingAverage-1)/movingAverage) + signalComponents.getTR());
		
		//plusDMn 
		signalComponents.setPlusDMn((prevSignalComponents.getPlusDMn() * (movingAverage-1)/movingAverage) + signalComponents.getPlusDM1());
		
		//minusDMn
		signalComponents.setMinusDMn((prevSignalComponents.getMinusDMn() * (movingAverage-1)/movingAverage) + signalComponents.getMinusDM1());
		...
		//ADX
		signalComponents.setADXn( ( ( prevSignalComponents.getADXn() * ( (movingAverage-1D)/movingAverage) ) + ( signalComponents.getDX()*(1D/movingAverage) ) ) * volumeWeightage );
